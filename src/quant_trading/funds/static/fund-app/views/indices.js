...
export async function mountIndices() {
  ...
  try {
    const data = await apiGet("/market-indices", { region: "all" });
    ...
    if (!rows) {
      rows =
        '<tr><td colspan="8">暂无指数数据，请确认 <code>market_index_daily_*</code> 爬虫已运行</td></tr>';
    }

    host.innerHTML = `<p class="sub meta">共 ${(data.items || []).length} 只指数 · 各标的最近交易日收盘 · 来源 <code>market_index_daily</code></p>
      <section class="panel table-scroll">
        <table class="data"><thead><tr>
          <th>代码</th><th>名称</th><th>市场</th><th>数据日</th><th class="num">收盘</th>
