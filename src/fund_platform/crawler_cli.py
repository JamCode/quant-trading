...
def _run_us_stock_daily_job() -> dict[str, Any]:
    return sync_us_stock_daily()
...
def main() -> None:
    ...
    scheduler.add_job(
        _scheduled("us_stock_daily_sync", _run_us_stock_daily_job),
        CronTrigger(
            day_of_week=fp_settings.us_stock_daily_cron_day_of_week(),
            hour=fp_settings.us_stock_daily_cron_hour(),
            minute=fp_settings.us_stock_daily_cron_minute(),
        ),
        id="us_stock_daily_sync",
        replace_existing=True,
        max_instances=1,
        coalesce=True,
    )
    registered.add("us_stock_daily_sync")
