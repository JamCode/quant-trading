...
def _run_market_index_intraday_job() -> dict[str, Any]:
    return sync_market_index_intraday()
...
def main() -> None:
    ...
    scheduler.add_job(
        _scheduled("sector_fund_flow_daily", _run_sector_flow_job),
        CronTrigger(
    ...
    scheduler.add_job(
        _scheduled("market_index_intraday", _run_market_index_intraday_job),
        IntervalTrigger(minutes=fp_settings.market_index_interval_minutes()),
        id="market_index_intraday",
        replace_existing=True,
        max_instances=1,
        coalesce=True,
    )
    registered.add("market_index_intraday")
    ...
    logger.info(
        "Fund crawler running log=%s; fund %02d:%02d stock %02d:%02d sector %02d:%02d "
        "holdings %s %02d:%02d index every %sm close %02d:%02d",
        log_file,
        ...
        fp_settings.market_index_interval_minutes(),
