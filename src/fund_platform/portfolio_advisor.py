...
分析方法论（按此思考，再落笔）：
- 先事实后观点：关键数据（涨跌幅、指数、事件）→ 归因（宏观/行业/指数β/主题β/估值/资金）→ 风险 → 机会。
- 回答用户真正关心的：「为什么涨、为什么跌」「有什么风险、什么机会」，而非空泛「值得关注」。
- 同主题多只基金可合并成一段，但须点明涉及哪些代码；每只基金至少出现一次，且说明近期涨跌方向。
...
【二、持仓涨跌归因】（核心，约 900 字）
目标：说清楚「为什么涨、为什么跌、风险、机会」。
要求：
1) 覆盖上方全部基金代码（可按下述主题合并叙述，但每只代码至少出现 1 次）：
   · A股科技/主题（通信、卫星、电网、机器人、半导体、科创、创业板等）
   · A股宽基/增强（如沪深300增强）
   · QDII·美股（标普、纳斯达克等）
   · QDII·港股（恒生、港股创新药等）
2) 每一只（或合并的一组，须列代码）按下面四行写，先检索该基金/跟踪指数近 1 周、近 1 月表现：
   · 近期表现：涨/跌 + 尽量给涨跌幅或排名（查不到写信息不足）
   · 上涨或下跌原因：产业链/指数/政策/估值/资金，讲清因果，区分「跟大盘」还是「主题独立」
   · 主要风险：未来 1～3 个月 1～2 条
   · 主要机会：未来 1～3 个月 1～2 条
...
def call_qwen_analysis(user_prompt: str) -> tuple[str, dict[str, Any]]:
    ...
    max_tokens = int(os.environ.get("FUND_ADVISOR_MAX_TOKENS", "2800"))
